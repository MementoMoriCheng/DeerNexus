...
| PR-025C+ | 解析器切 Membership-based org 解析（仅 HTTP 路径） | — | 待合并 | 新建 `deerflow/tenancy/membership.py::get_active_membership`（单 membership 严格：0→None、1→row、>1→`MultiMembershipError`；命中 `idx_org_memberships_user_status`）；`resolve_tenant_context` 改 `async` + `current_multi_org_phase()` 门禁（`disabled`=快速单 Org 无 DB，可回滚；`validation`/`active`=经 membership 解析 org）；middleware 一行 `await`，fail-closed 503 wrapper 不变；**仅 HTTP 路径**（channel dispatch 的 `resolve_channel_tenant_context`/`channel_tenant_scope` 保持单 Org 同步不动）；TEN-008 保持（无 membership 不合成默认 org）；backend=memory sf=None fail-closed；详见 runtime-contracts §16.21 | — |
