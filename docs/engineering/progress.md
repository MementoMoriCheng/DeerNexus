...
| PR-025B–D | Multi-org Feature + doctor + Contract | — | 未开始（PR-025A 已交付，schema Enforce 就绪） | 不可回滚发布；Feature Flag + 验证 Org（B，Flag 机制从零引入，ci-cd §10.3 门禁 A 上线+双 Org 矩阵全绿+空 org_id=0）、doctor 启用与迁移阶段探测（C）、移除 `user_id` 分支 + 清理临时兼容索引 + org-scope 现有全局唯一（D，Contract 至少晚一个稳定窗口） |
