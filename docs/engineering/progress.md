...
| PR-025A–D | Enforce + Multi-org Feature | — | 未开始（PR-024 已交付，前置 schema/backfill/scope 就绪） | 不可回滚发布；`org_id NOT NULL` + 复合唯一（A）、Feature Flag + 验证 Org（B）、doctor 启用（C）、移除 `user_id` 分支 + 清理临时兼容索引（D） |
