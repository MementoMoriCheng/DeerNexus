...
- [~] 最小 9 类事件全部产生（PR-044：`auth.login`（成功+失败）+ `policy.tool.denied`（RBAC + 工具护栏双维度）落地，加 PR-042 的 `iam.role_binding.created/deleted` + PR-053 的 `release.agent.published`/`release.agent.rolled_back`（ReleaseChannel CAS）+ PR-043 的 `catalog.skill.changed`/`catalog.mcp.changed`（skills.py / mcp.py 写路径），共 **8/9**；唯一缺 `policy.approval.required`（无生产者）。注：`catalog.skill.changed`/`catalog.mcp.changed` 目标代码（skills.py/mcp.py）是文件持久化，文件 IO 不在 DB 事务内，故走 best-effort Class B（`emit_class_b_audit` durable pending 行，PR-043），非 Class A 同事务；待 skills/mcp 多租户化迁 DB 后升级 Class A）
- [~] IAM、API Key、Connector、Release 关键写路径覆盖（PR-042：IAM 全覆盖 —— ServiceAccount 生命周期 / RoleBinding / API Key / OrgMembership / OIDC group mapping 共 14 个写路径全部同事务 enqueue；action 归一化为 `<domain>.<resource>.<verb>` 注册表。Release 写路径已由 Track E 全覆盖 —— PR-052 制品 CRUD/review/publish/revoke + PR-051 文件态导入 + PR-053/055 promote/rollback 共 11 个写端点全部 Class A 同事务 enqueue（`agent_artifacts.py`）。Skill/MCP 写路径由 PR-043 best-effort Class B 覆盖（文件持久化限制）。**Connector 写路径仍缺** —— 无 connector router 存在，`connector.configuration.changed`/`connector.secret_reference.changed` 待 connector 域代码落地）
