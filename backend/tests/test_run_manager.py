...
class FlakyStatusRunStore(MemoryRunStore):
    ...
    async def update_status(self, run_id, status, *, error=None, expected_row_version=None):
        self.status_update_attempts += 1
        if self.status_failures > 0:
            self.status_failures -= 1
            raise sqlite3.OperationalError("database is locked")
        return await super().update_status(run_id, status, error=error, expected_row_version=expected_row_version)
...
class MissingRowStatusRunStore(MemoryRunStore):
    ...
    async def update_status(self, run_id, status, *, error=None, expected_row_version=None):
        await super().update_status(run_id, status, error=error, expected_row_version=expected_row_version)
        return False
