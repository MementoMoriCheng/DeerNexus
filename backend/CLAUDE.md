...
- `tests/test_harness_boundary.py` — ensures `packages/harness/deerflow/` never imports from `app.*`, and that `packages/harness/deerflow/contracts/` only depends on the standard library and `pydantic` (no ORM, FastAPI, LangGraph/LangChain, or `deerflow.*` business subpackages). The contracts guard uses an allow-list and fails closed, so adding a dependency requires an explicit review recorded against [runtime-contracts.md §2](../docs/architecture/runtime-contracts.md). Authoritative contract DTOs, Protocols, the error code registry, and canonical JSON fixtures live under `packages/harness/deerflow/contracts/` (identity/context/errors from PR-010; policy/release/runs/approval/events + `PolicyEvaluator`/`ReleaseResolver`/`AuditSink`/`UsageRecorder` Protocols from PR-011); see `tests/test_contracts_base.py` (markers `CONTRACT-010-*`), `tests/test_contracts_policy_release_event.py` (markers `CONTRACT-011-*`), and `tests/fixtures/contracts/`.
